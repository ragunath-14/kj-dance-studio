...
async function runPendingFeeAlerts() {
  ...
  try {
    ...
    for (const student of students) {
      ...
      // Normalize joinDate to start of day for inclusive comparison
      ...
      if (student.isActive === false) {
        if (isAnniversary) {
          console.log(`  📣 Anniversary detected for Inactive student: ${student.studentName}. Sending rejoin invitation...`);
          try {
            const result = await whatsapp.sendRejoinMessage(student, student.studentName, student.classType);
            if (result.success) {
              console.log(`  📲 Rejoin message sent → ${student.studentName}`);
              rejoinSent++;
            } else {
              console.warn(`  ⚠️  Rejoin message failed → ${student.studentName}: ${result.reason}`);
            }
          } catch (err) {
      ...
      const totalPaid = payments
        .filter(p => {
          const sid = p.studentId?._id?.toString() || p.studentId?.toString();
          if (sid !== student._id.toString()) return false;
          
          const pDate = new Date(p.date);
          const pDateOnly = new Date(pDate.getFullYear(), pDate.getMonth(), pDate.getDate());
          return pDateOnly >= joinDateStart;
        })
        .reduce((sum, p) => sum + (p.amount || 0), 0);

      // Calculate months since joining (inclusive of start month)
      let totalCycles =
      ...
      // ...OR if it's been more than 3 days since the last alert
      ...
      try {
        ...
        if (result.success) {
          console.log(`  📲 Alert sent → ${student.studentName} — ₹${totalDue} due`);
